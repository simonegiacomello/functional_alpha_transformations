...
modified_date_dataset = read.csv("RawData/comuni_giornaliero_31dicembre.txt", header = T)
...
source("Scripts/Utility_Functions.R")
provinces_data = read.csv("RawData/Provinces_Population_2020.txt")
provinces_data = provinces_data %>% dplyr::select(Provincia, Età, Totale.Maschi, Totale.Femmine) %>% 
  filter(Età != "Totale") %>% mutate(provincia = Provincia, num_residenti = Totale.Maschi + Totale.Femmine) %>% 
  dplyr::select(provincia,num_residenti, Età,Totale.Maschi, Totale.Femmine)
provinces_data$Età = as.numeric(provinces_data$Età)
provinces_data = provinces_data %>% mutate(CL_ETA = age_to_fact(Età)) %>% dplyr::select(-Età)
