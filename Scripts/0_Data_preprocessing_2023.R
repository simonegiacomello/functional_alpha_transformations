...
modified_date_dataset = read.csv("RawData/comuni_giornaliero_31dicembre23.csv", header = T)
...
save(modified_date_dataset, file = Rdata_path("modified_istat_dataset23")) 
...
load("Output/Data/modified_istat_dataset23.Rdata")
...
provinces_data = modified_date_dataset %>% group_by(NOME_PROVINCIA,CL_ETA,partial_date_death) %>% 
  summarise_at(c(paste0("M_",c(11:23)),paste0("F_",c(11:23)),paste0("T_",c(11:23))), sum) 
provinces_aggregated = aggregate_classes(provinces_data,keyname = "NOME_PROVINCIA",firstvec,lastvec)
save(list = c("provinces_aggregated","provinces_names"), file = Rdata_path("Aggregated_Provinces_23"))
...
municipality_data = municipality_data %>% dplyr::select(Codice.comune, EtÃƒ., Totale.Maschi, Totale.Femmine) %>% 
  filter(EtÃƒ. != 999) %>% mutate(istat = Codice.comune, num_residenti = Totale.Maschi + Totale.Femmine) %>% 
  dplyr::select(istat,num_residenti, EtÃƒ.,Totale.Maschi, Totale.Femmine)

### Now we perform aggregation, see factorization function in Utility_functions, which is written according to our aggregation
municipality_data = municipality_data %>% mutate(CL_ETA = age_to_fact(EtÃƒ.)) %>% dplyr::select(-EtÃƒ.)
...
# provinces_data = provinces_data %>% dplyr::select(Provincia, EtÃƒ., Totale.Maschi, Totale.Femmine) %>% 
#   filter(EtÃƒ. != "Totale") %>% mutate(provincia = Provincia, num_residenti = Totale.Maschi + Totale.Femmine) %>% 
#   dplyr::select(provincia,num_residenti, EtÃƒ.,Totale.Maschi, Totale.Femmine)
# provinces_data$EtÃƒ. = as.numeric(provinces_data$EtÃƒ.)
# provinces_data = provinces_data %>% mutate(CL_ETA = age_to_fact(EtÃƒ.)) %>% dplyr::select(-EtÃƒ.)
...
# 
# provinces_data$provincia[provinces_data$provincia == "Valle d'Aosta/VallÃƒÂ©e d'Aoste"] = "Valle d'Aosta/VallÃ©e d'Aoste"
# provinces_data$provincia[provinces_data$provincia == "Reggio di Calabria"] = "Reggio Calabria"
# provinces_data$provincia[provinces_data$provincia == "ForlÃƒÂ¬-Cesena"] = "ForlÃ¬-Cesena"

save(provinces_aggregated,provinces_names, provinces_data, file = Rdata_path("Aggregated_Provinces_23"))
...
prov_poly$DEN_PCM[prov_poly$DEN_PCM == "Aosta"] = "Valle d'Aosta/VallÃ©e d'Aoste"
prov_poly$DEN_PCM[prov_poly$DEN_PCM == "Bolzano"] = "Bolzano/Bozen" 
prov_poly$DEN_PCM[prov_poly$DEN_PCM == "Massa Carrara"] = "Massa-Carrara" 
prov_poly$DEN_PCM[prov_poly$DEN_PCM == "Forli'-Cesena"] = "ForlÃ¬-Cesena" 
