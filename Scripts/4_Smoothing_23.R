...
data_to_smooth_provinces = data_to_smooth_provinces %>% mutate_at(.vars = paste0("T_",11:23),.funs = function(x){return(x/sum(x))})

save(data_to_smooth_provinces, file = Rdata_path("Smoothing_datasets_23"))


######### smoothing without 0 imputation

load("Output/Data/Smoothing_datasets_23.Rdata")
...
clusterExport(cl, 
              c("xcp_extended", "alfa", "w", "ch", "der", "degree", "knots_vector_extended",
                ...
                "smooth.basis", "eval.fd", "fdPar"))
registerDoParallel(cl)

load("Output/Data/Smoothing_Provinces_52_alpha.Rdata")  #load smoothed data up to 2020


#add smoothed data up to 2023
...
for (year in years[11:13]) flist_province[[year]] = alpha.folding(clist_province[["0.5"]][[year]], 0.5)

save(clist_province,flist_province, file = Rdata_path(paste0("Smoothing_Provinces_",knots,"_alpha_23")))
...
load("Output/Data/Smoothing_Provinces_52_alpha_isometric.Rdata")  #load smoothed data up to 2020
...
for (year in years[11:13]) flist_province[[year]] = alpha.isometric.inv(clist_province[["0.5"]][[year]], 0.5)

save(clist_province, flist_province, file = Rdata_path(paste0("Smoothing_Provinces_",knots,"_alpha_isometric_23")))
