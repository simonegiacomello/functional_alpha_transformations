...
models.fitting = function(alpha, train.indexes, test.indexes) {
  ...
  #fit the models
  for(i in (length(years)-2):length(years))
  {
    year = years[i]
    print(year)
...
load("Output/Data/Smoothing_Provinces_52_alpha_23.Rdata")  #smoothed curves
load("Output/Data/Indicators_cv_Tsagris_20.Rdata")  #up to 2020 cv data
...
for (alpha in alpha.seq) {
  ...
  idx = paste0(alpha)
  data = data.frame(Year = rep(years_names[(length(years)-2):length(years)], each=nfolds*3), 
                    Indicator = rep(c("R^2", "MedAPE","KL-divergence"), each=nfolds, times=3),
                    Value = NA)
  
  cv.indicators = cross.validation.indicators(nfolds, alpha)
  
  for (year in years[11:13]) {
  ...
  indicators_data[[idx]] = rbind(indicators_data[[idx]], data)
  
}



save(indicators_data, file = Rdata_path("Indicators_cv_Tsagris_23"))
...
load("Output/Data/Indicators_cv_Isometric_20.Rdata")  #up to 2020 cv data
...
for (alpha in alpha.seq) {
  ...
  cv.indicators = cross.validation.indicators(nfolds, alpha, transformation = "Isometric")
  ...
  indicators_data_isom[[idx]] = rbind(indicators_data_isom[[idx]], data)
  
}

save(indicators_data_isom, file = Rdata_path("Indicators_cv_Isometric_23"))
...
ggplot(temp, aes(x = as.factor(Year), y = Value, fill = Transformation)) +
  ...
  labs(x = "Year", y = "Value", title = 
         TeX(paste0("$R^2$ comparison. Tsagris $\\alpha =", idx.best,"$, Isometric $\\alpha = ",idx.best_isom,"$"))) +
  theme_minimal() +
...
ggplot(temp, aes(x = as.factor(Year), y = Value, fill = Transformation)) +
  ...
  geom_vline(xintercept = seq(1.5, length(unique(temp$Year)) - 0.5, by = 1), linetype = "dashed", color = "grey") +
  labs(x = "Year", y = "Value", title = 
         TeX(paste0("MedAPE comparison. Tsagris $\\alpha =", idx.best,"$, Isometric $\\alpha = ",idx.best_isom,"$"))) +
  theme_minimal() +
  theme(legend.position = "bottom") +
  scale_fill_manual(values = c("CLR" = "skyblue", "Tsagris" = "orange", "Isometric" = "red"))
...
ggplot(temp, aes(x = as.factor(Year), y = Value, fill = Transformation)) +
  ...
  geom_vline(xintercept = seq(1.5, length(unique(temp$Year)) - 0.5, by = 1), linetype = "dashed", color = "grey") +
  labs(x = "Year", y = "Value", title = 
         TeX(paste0("KL-divergence comparison. Tsagris $\\alpha =", idx.best,"$, Isometric $\\alpha = ",idx.best_isom,"$"))) +
  theme_minimal() +
  theme(legend.position = "bottom") +
  scale_fill_manual(values = c("CLR" = "skyblue", "Tsagris" = "orange", "Isometric" = "red"))
